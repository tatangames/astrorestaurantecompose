...
import com.tatanstudios.astropollococina.model.ordenes.ModeloOrdenesPreparacionArray
import com.tatanstudios.astropollococina.model.rutas.Routes

@OptIn(ExperimentalMaterialApi::class)
@Composable
fun ListadoPreparacionOrdenScreen(navController: NavHostController,
                           viewModel: OrdenPreparacionBuscarViewModel = viewModel()
) {
    ...
    var modeloListaOrdenesPreparacionArray by remember { mutableStateOf(listOf<ModeloOrdenesPreparacionArray>()) }

    val refreshing by remember { mutableStateOf(false) }
    val pullRefreshState = rememberPullRefreshState(
        refreshing = refreshing,
        onRefresh = {
            viewModel.prepracionOrdenRetrofit(_idusuario)
        }
    )
    ...
    Scaffold(
        ...
        Box(
            ...
            if (modeloListaOrdenesPreparacionArray.isEmpty() && boolDatosCargados) {
        ...
        resultado?.getContentIfNotHandled()?.let { result ->
            when (result.success) {
                1 -> {
                    modeloListaOrdenesPreparacionArray = result.lista
